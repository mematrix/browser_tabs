...
impl Default for QtUIConfig {
    fn default() -> Self {
        Self {
            enable_system_tray: true,
            enable_hotkeys: true,
            enable_notifications: true,
            organization_name: "WebPageManager".to_string(),
            application_name: "WebPageManager".to_string(),
...
impl std::fmt::Debug for QtUIState {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("QtUIState")
...
pub struct QtUIManager {
    ...
    config: QtUIConfig,
    state: Arc<RwLock<QtUIState>>,
}

impl QtUIManager {
    pub fn new() -> Self {
        Self::with_config(QtUIConfig::default())
    }
    
    pub fn with_config(config: QtUIConfig) -> Self {
        let initial_theme = config.initial_theme;
        Self {
            initialized: std::sync::atomic::AtomicBool::new(false),
            config,
            state: Arc::new(RwLock::new(QtUIState {
    ...
    pub fn config(&self) -> &QtUIConfig {
        &self.config
    }
}

impl Default for QtUIManager {
    fn default() -> Self {
        Self::new()
    }
}
...
impl UIManager for QtUIManager {
    ...
    fn hide_main_window(&self) -> Pin<Box<dyn Future<Output = Result<()>> + Send + '_>> {
        ...
        Box::pin(async move {
            ...
            tracing::info!("Hiding Qt main window");
    ...
    fn show_notification(&self, notification: &NotificationConfig) -> Pin<Box<dyn Future<Output = Result<()>> + Send + '_>> {
        ...
        Box::pin(async move {
            ...
            tracing::info!("Showing Qt notification: {} - {}", notification.title, notification.message);
    ...
    fn unregister_global_hotkeys(&self) -> Pin<Box<dyn Future<Output = Result<()>> + Send + '_>> {
        let state = self.state.clone();
        Box::pin(async move {
            tracing::info!("Unregistering all Qt global hotkeys");
    ...
    fn restore_from_tray(&self) -> Pin<Box<dyn Future<Output = Result<()>> + Send + '_>> {
        let state = self.state.clone();
        Box::pin(async move {
            tracing::info!("Restoring from Qt system tray");
    ...
    fn set_theme(&self, theme: UITheme) -> Pin<Box<dyn Future<Output = Result<()>> + Send + '_>> {
        let state = self.state.clone();
        Box::pin(async move {
            tracing::info!("Setting Qt theme to {:?}", theme);
    ...
    fn set_event_handler(&self, handler: Arc<dyn UIEventHandler>) -> Pin<Box<dyn Future<Output = Result<()>> + Send + '_>> {
        let state = self.state.clone();
        Box::pin(async move {
            tracing::debug!("Setting Qt event handler");
